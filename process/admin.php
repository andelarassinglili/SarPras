...
if (isset($_POST['ubah'])) {
    ...
    if ($password != "") {
        $update = mysqli_query($con, "UPDATE users SET nama='$nama', username='$username', no_hp='$no_hp', password='" . password_hash($password, PASSWORD_DEFAULT) . "', level='$level', id_sekolah='$id_sekolah' WHERE id_users='$id'") or die(mysqli_error($con));
    } else {
